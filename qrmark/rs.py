r"""
A pure-python Reed Solomon encoder/decoder, based on the wonderful tutorial at
...
the message, where ``nsym`` is the number of bytes in the error correction code.
    ...
    'hello world'
    >>> rs.decode('hello worXd\xed%T\xc4\xfd\xfd\x89\xf3\xa8\xaa')  # 1 error
    'hello world'
    >>> rs.decode('heXlo worXd\xed%T\xc4\xfdX\x89\xf3\xa8\xaa')     # 3 errors
    'hello world'
    >>> rs.decode('hXXlo worXd\xed%T\xc4\xfdX\x89\xf3\xa8\xaa')     # 4 errors
    'hello world'
    >>> rs.decode('hXXXo worXd\xed%T\xc4\xfdX\x89\xf3\xa8\xaa')     # 5 errors
    'hello world'
    ...
    >>> rs.decode('hello worXXXXy\xb2XX\x01q\xb9\xe3\xe2=')         # 6 errors - ok
    'hello world'
...
class RSCodec(object):
    ...
    def encode(self, data):
        ...
        enc = rs_encode_msg([ord(ch) for ch in data], self.nsym)
        if PY3:
            return bytes(enc)
        else:
    ...
    def decode(self, data):
        dec = rs_correct_msg([ord(ch) for ch in data], self.nsym)
        if PY3:
            output = bytes(dec)
        else:
