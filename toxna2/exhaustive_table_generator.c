...
int digit_term_get_limit(int count)
{
...
int latin_term_get_rec(int count, int k, char * output);

int latin_term_get_limit(int count)
{
	/* sum of geometric series */
	return (1-longpow(26, count+1)) / (-25) - 1;
}

int latin_term_get(int count, int k, char * output)
{
...
int ascii_term_get_limit(int count)
{
...
/* ============================== LEXICON TERM ============================= */
int lexicon_term_get_rec(const char ** lexicon, int num_of_words, int count, int k, char * output);

int lexicon_term_get_limit(int num_of_words, int count)
{
	/* sum of geometric series */
	return (1-longpow(num_of_words, count+1)) / (-num_of_words+1) - 1;
}

int lexicon_term_get(const char ** lexicon, int num_of_words, int count, int k, char * output)
{
...
int lexicon_term_get_rec(const char ** lexicon, int num_of_words, int count, int k, char * output)
{
	...
	if (k < max) {
		...
		return lexicon_term_get_rec(lexicon, num_of_words, count - 1, k - max, output + strlen(lexicon[0]));
	}
}
...
int rule_load_lexicon(rule_info_t * info, const char* filename)
{
	char line[200];
	int i;
...
int rule_load_pattern(rule_info_t * info, const char* pattern)
{
	...
	info->terms = (term_info_t*)malloc(sizeof(term_info_t) * info->num_of_terms);
	if (info->terms == NULL) {
	...
	for (i = j = 0; i < pattern_len; i += 2, j++) {
		...
		switch(pattern[i]) {
			case '#':
				...
				info->terms[j].limit = digit_term_get_limit(info->terms[j].count);
				break;
			case '.':
				info->terms[j].type = TERM_LATIN;
				info->terms[j].limit = latin_term_get_limit(info->terms[j].count);
				break;
			case '$':
				info->terms[j].type = TERM_ASCII;
				info->terms[j].limit = ascii_term_get_limit(info->terms[j].count);
				break;
			case '@':
				info->terms[j].type = TERM_LEXICON;
				info->terms[j].limit = lexicon_term_get_limit(info->num_of_words, info->terms[j].count);
				break;
			default:
				// invalid syntax
				printf("invalid rule pattern\n");
...
int rule_load(rule_info_t * info, const char * pattern, const char * lexfilename, const char * hashname, const char * flag)
{
	if (stricmp(flag, "all") == 0) {
		info->limit = -1;
	}
	else {
	...
	if (stricmp(hashname, "md5") == 0) {
		info->hashfunc = MD5BasicHash;
	}
	else if (stricmp(hashname, "sha1") == 0) {
...
int rule_generate_incrementing(rule_info_t * info, char * output)
{
	int i;
	int succ;
	...
	for (i = 0; i < info->num_of_terms; i++) {
		...
		switch (info->terms[i].type) {
			case TERM_DIGIT:
				succ = digit_term_get(info->terms[i].count, info->terms[i].k, output);
				break;
			case TERM_LATIN:
				succ = latin_term_get(info->terms[i].count, info->terms[i].k, output);
				break;
			case TERM_ASCII:
				succ = ascii_term_get(info->terms[i].count, info->terms[i].k, output);
				break;
			case TERM_LEXICON:
				succ = lexicon_term_get(info->words, info->num_of_words, 
					info->terms[i].count, info->terms[i].k, output);
				break;
		}
		if (succ != 0) {
...
int rule_generate_random(rule_info_t * info, char * output)
{
	int i;
	int succ;
	int k;
...
int main(int argc, const char** argv)
{
	...
	rule_load(&rule, "#2@1", "../MyLexicon.txt", "md5", "8");
